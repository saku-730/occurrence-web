...
最重要原則は「1 Occurrence = 1分類群 × 1地点」です。
1つのOccurrenceに複数の地点をまとめてはいけません。
また、scientificName に属名の頭文字だけを使った省略学名を出力してはいけません。
...
### 複数地点の分割例
論文に次のようにある場合:
`Pheretima agrestis は奈良市4か所（大慈仙町・奈良公園・南京公団・奈良市真美ヶ丘）で記録された。`
