...
///
/// A single query is built from the occurrence's existing Darwin Core location fields,
/// with locality as the most specific component. Nominatim failures deliberately fail open
/// so occurrence registration can continue without generated coordinates.
pub async fn geocoding_middleware(request: Request, next: Next) -> Response {
    ...
    let rewritten = if is_batch {
        geocode_batch_json(&original, geocoder).await
    } else {
        match enrich_nquads_with_geocoding(&original, geocoder).await {
            Ok(bytes) => bytes,
    ...
    G: LocationGeocoder + ?Sized,
{
    ...
    let result = match geocoder.geocode(&query).await {
        ...
        Ok(None) | Err(_) => return Ok(nquads.to_vec()),
    };
...
_:o <http://rs.tdwg.org/dwc/terms/locality> "Kyoto" <https://bio-database.net/graphs/occurrences> ."#;
    ...
    #[tokio::test]
    async fn generated_coordinates_and_only_nominatim_source_are_added() {
