...
frontend から送信される N-Quads には graph name を含めてはならない。  
graph name が含まれていた場合、差し替えずに 400 で拒否する。

backend は保存前に occurrence graph を付与する。
...
```nq
_:occurrence <https://example.org/predicate> "value" .
_:occurrence <https://example.org/another> <https://example.org/object> .
```
...
```nq
_:a <https://example.org/predicate> "x" .
_:b <https://example.org/predicate> "y" .
```

object blank node。

```nq
_:occurrence <https://example.org/predicate> _:object .
```
