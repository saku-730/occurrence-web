...
詳細な現行実装、暫定保存形式、既知の制約、PostgreSQL/APIの設計、実装順序、テスト要件は [`20_label_templates.md`](20_label_templates.md) を正本とする。

現在はラベル作成とレイアウト編集を統合し、実データを見ながら表示項目・寸法・文字サイズを調整してA4・個別プレビュー、PDF、印刷へ反映できる。設定はダイアログ内だけで保持する。独立作成画面とlocalStorage保存は廃止した。PostgreSQL永続化、所有者認可、CRUD API、名前付き設定の保存・選択・編集・削除は今後この統合画面へ追加する。
